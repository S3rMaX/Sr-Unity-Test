...
C. The Evaluate method is removed, and instead, a Func<T, bool> parameter is added to the FilterCollection method signature. This parameter will be used as a predicate function to determine whether or not an element should be included in the filtered collection. The predicate function can be defined at runtime when the FilterCollection method is called.
...
E.The FilterCollection method is implemented as an iterator block that uses the yield keyword to return elements one at a time. This makes it a lazy collection, because it only evaluates the predicate function for each element as it is requested, rather than eagerly evaluating the entire collection upfront.
**/